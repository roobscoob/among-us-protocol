...
| Type | Name | Description |
| --- | --- | --- |
| `byte` | Is Forced | While this byte is similar to a `boolean`, it has different conditions:<br><ul><li>If this value is anything but `1` then the `Disconnect Reason` below will be parsed and displayed</li><li>If this value is exactly `1` then the `Disconnect Reason` below will not be read and the game will display the message *"Forcible disconnected from the server"*.</li></ul> |
